...
ProcessorMap::ProcessorMap(core::IArena& arena)
...
#ifdef ROC_TARGET_SPEEXDSP
    {
        ...
        node->ctor_fn = (void*)(ResamplerFunc)&resampler_ctor<SpeexResampler>;
    ...
    }
    {
        ...
        node->ctor_fn = (void*)(ResamplerFunc)&resampler_dec_ctor<SpeexResampler>;
        register_builtin_node_(node);
    }
#endif // ROC_TARGET_SPEEXDSP
    {
        ...
        node->ctor_fn = (void*)(ResamplerFunc)&resampler_ctor<BuiltinResampler>;
        register_builtin_node_(node);
    }
}
...
core::SharedPtr<IResampler> ProcessorMap::new_resampler(core::IArena& arena,
                                                        FrameFactory& frame_factory,
                                                        const ResamplerConfig& config,
                                                        const SampleSpec& in_spec,
                                                        const SampleSpec& out_spec) {
    core::SharedPtr<Node> node = find_node_(NodeType_Resampler, config.backend);
    if (!node) {
    ...
    return ((ResamplerFunc)node->ctor_fn)(arena, frame_factory, config.profile, in_spec,
                                          out_spec);
}
